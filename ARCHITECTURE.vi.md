...
### 8.1. Web — sáu mode phân quyền (cùng một backend)

`src/web/server.ts` bật một trong sáu mode qua env `BOW_*_MODE`; mỗi mode một **cổng riêng**
(`BOW_AGENT_PORT`) nên chạy song song không đụng nhau. Các mode chia sẻ đặt
`BOW_SERVE_STATIC=true` → backend tự phục vụ frontend đã build ngay trên cổng đó (riêng Dev vẫn
có Vite dev server ở 5173). Chính sách nằm trong `canUseTool` (`runner.ts`) + middleware
`checkReadonlyConfig`/`requireAdmin` (`server.ts`):

| Mode | Cổng | Ép mode | Chính sách |
|---|---|---|---|
| **Dev** (`ui`) | 4000 (+ Vite 5173) | client chọn | Admin (localhost) full. **Client LAN non-admin bị ép `plan`** (read-only) — muốn ghi phải qua Collab |
| **QC** (`ui:qc:share`, `BOW_QC_MODE`) | 4001 | `plan` | **WHITELIST** tool đọc (Read/Glob/MCP-read) **+ Skill** (kích hoạt qc-triage) **+ Jira read/write** (comment/transition); Grep DENY riêng; file nhạy cảm chặn; source code DENY; ép model `claude-sonnet-5`; ẩn UI kỹ thuật |
| **Collab** (`ui:collab`, `BOW_COLLAB_MODE`) | 4002 | `auto` | CTV sửa code + chạy test tự do; **mọi thao tác GHI (Edit/Write, Bash rủi ro kể cả Git, MCP write) của CTV non-admin bị định tuyến lên ADMIN duyệt từ xa** (`requireApprovalForWrites` + `adminBus`) |
| **BA** (`ui:ba`, `BOW_BA_MODE`) | 4003 | `auto` | Ghi **tài liệu** (`isDocPath`: `docs/`, `*.md/.mdx/.txt`) + **full Jira** tự do; source code / config / DB / deploy **DENY CỨNG** (không hỏi admin — muốn sửa code thì đổi mode) |
| **Reviewer** (`ui:review:share`, `BOW_REVIEWER_MODE`) | 4004 | `plan` | **WHITELIST** tool đọc + Skill (pr-review) + **Bash** lọc riêng (`isReviewGhCommand`: `git diff/status/log/show`, `gh pr view/diff/list/checks/comment/review`) + `SAFE_COMMANDS` (test/analyze); Jira **đọc**; ghi file source / merge / push / risky / command-chaining **DENY**; ép `claude-sonnet-5`; ẩn UI kỹ thuật |
| **DevOps** (`ui:devops:share`, `BOW_DEVOPS_MODE`) | 4005 | `auto` | Mode **LAI**: ghi **file hạ tầng** theo đích (`isInfraPath`: Dockerfile, `docker-compose*`, `.github/workflows/*`, `*.tf/*.hcl`, k8s/Helm) + docs vận hành như BA; nhưng **source ứng dụng DENY CỨNG**, và **lệnh deploy/apply treo admin duyệt** như Collab (`routeToAdmin = (isCollabMode \|\| isDevOpsMode) && !isAdmin`). Lệnh sửa file tại chỗ qua Bash (`sed -i`, `patch`, `git apply`) LUÔN phải duyệt — Bash không được là đường vòng qua deny cứng của `Edit` |
