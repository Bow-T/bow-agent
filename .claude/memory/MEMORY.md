...
- [DevOps Mode (Triển khai & Hạ tầng)](bow-devops-mode.md) — npm run ui:devops:share (4005/5178): ghi FILE HẠ TẦNG (Dockerfile/compose/workflows/*.tf/k8s/Helm) + docs, DENY cứng source ứng dụng; deploy/apply treo admin duyệt (mode LAI: isInfraPath như BA + routeToAdmin như Collab). Repo skill bow-skill-devops CHƯA có (fail-open)
