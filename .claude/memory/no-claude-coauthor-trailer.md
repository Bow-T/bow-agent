...
**Chống lưng bằng hook:** đã có hook `PreToolUse(Bash)` tại `.claude/hooks/block-claude-coauthor.py` (đăng ký trong `.claude/settings.json`) tự CHẶN mọi `git commit`/`git merge` có dòng ghi công Claude — cả trailer `Co-Authored-By: ...Claude/Anthropic` LẪN footer `Generated with Claude Code` / link `claude.com/claude-code`. Nếu bị chặn (exit 2), viết lại message bỏ hẳn dòng đó rồi commit lại — đừng tìm cách lách hook. Mẹo khi amend: viết message sạch ra file rồi `git commit -F <file>` để lệnh không nhúng chuỗi cấm (nếu không, chính lệnh commit của bạn cũng bị hook chặn).
