...
description: MCP riêng theo user (overlay lên MCP chung) — user LAN tự quản, kể cả QC/Collab
metadata:
  type: project
---
...
User LAN (qc/collab) muốn MCP riêng (token/DB riêng) thì đụng của mọi người. Thêm nữa
`POST/DELETE /api/mcp` bị `requireAdmin` + `checkReadonlyConfig` chặn → họ còn chẳng sửa được.
...
- **`src/core/runner.ts`**: `RunOptions.userMcpServers?: Record<string,McpServerConfig>`.
  ...
  `QC_MODE_ALLOWED_TOOLS` (nên MCP riêng cũng đọc được trong QC Mode; tool GHI vẫn duyệt).
- **`src/web/server.ts`**: trong `/api/run` resolve `userMcpServers` từ token
  ...
  `requireAdmin`/`checkReadonlyConfig` nên **chạy được cả trong QC/Collab**. `/api/mcp` chung
  giữ nguyên (admin only, vẫn 403 trong QC/Collab — ranh giới cũ không đổi).
- **`web/App.tsx`**: nút MCP hiện cho admin (mọi mode trừ QC) HOẶC user LAN đã duyệt
