...
## 🐙 2. Quy Trình Git & Commit "Octopus Mode"
...
graph TD
    ...
    B --> C[Chạy quét an toàn: bash scripts/octopus-commit.sh]
    C --> D[Chạy phân tích Flutter: fvm flutter analyze]
    D --> E[Tạo commit message chuẩn Octopus]
    E --> F[Thực hiện commit & push lên branch]
```

### Chi Tiết Các Bước:

1.  **Quét an toàn (Safety Scan):** Chạy `bash scripts/octopus-commit.sh` để kiểm tra RLS, CORS, Secrets và views.
    > [!WARNING]
